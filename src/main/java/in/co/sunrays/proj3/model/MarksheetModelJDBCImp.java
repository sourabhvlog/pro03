...
public class MarksheetModelJDBCImp implements MarksheetModelInt {
	  ...
	  Logger log = Logger.getLogger(MarksheetModelJDBCImp.class);
	    ...
	    public long add(MarksheetDTO bean) throws ApplicationException,
	        ...
	        // get Student Name
	        StudentModelJDBCImp sModel = new StudentModelJDBCImp();
	        StudentDTO studentbean = sModel.findByPK(bean.getStudentId());
	        bean.setName(studentbean.getFirstName() + " "
	                + studentbean.getLastName());

	        MarksheetDTO duplicateMarksheet = findByRollNo(bean.getRollNo());
	        int pk = 0;
	     ...
	     * @throws DatabaseException
	     */
	    public void delete(MarksheetDTO bean) throws ApplicationException {
	    ...
	    public MarksheetDTO findByRollNo(String rollNo)
	        ...
	        StringBuffer sql = new StringBuffer(
	                "SELECT * FROM ST_MARKSHEET WHERE ROLLNO=?");
	    ...
	    public void update(MarksheetDTO bean) throws ApplicationException,
	            DuplicateRecordException {

	        log.debug("Model update Started");

	        Connection conn = null;
	        MarksheetDTO beanExist = findByRollNo(bean.getRollNo());
	        ...
	        try {
	            ...
	            conn.setAutoCommit(false); // Begin transaction
	            PreparedStatement pstmt = conn
	                    .prepareStatement("UPDATE ST_MARKSHEET SET ROLLNO=?,STUDENT_ID=?,NAME=?,PHYSICS=?,CHEMISTRY=?,MATHS=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
	    ...
	    public List search(MarksheetDTO bean) throws ApplicationException {
	    ...
	    public List search(MarksheetDTO bean, int pageNo, int pageSize)
	            throws ApplicationException {
	    ...
	    public List getMeritList(int pageNo, int pageSize)
	        ...
	        ArrayList list = new ArrayList();
	        StringBuffer sql = new StringBuffer(
	                "SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMISTRY`, `MATHS` , (PHYSICS + CHEMISTRY + MATHS) as total from ST_MARKSHEET where physics>=35 and chemistry>=35 and maths>=35 order by total desc");
	        // if page size is greater than zero then apply pagination
	        if (pageSize > 0) {
	            // Calculate start record index
	            pageNo = (pageNo - 1) * pageSize;
